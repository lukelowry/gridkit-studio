...
import { changeReal, faultRealEdits, textEdits } from '../src/gridkit/edit.js'
...
it('formats integer fault literals without changing their value or other case text', () => {
  ...
  const formatted = applyEdits(text, faultRealEdits(text, raw))
  expect(formatted).toBe(original)
  expect(JSON.parse(formatted)).toEqual(raw)
  expect(faultRealEdits(formatted, raw)).toEqual([])
})
